...
class Account extends CI_Controller {
    ...
    public function addCashTransaction()  {
        ...
        try{
            ...
            $res = ['success'=>true, 'message'=>'Transaction added','paymentId'=>$paymentId];
        } catch (Exception $ex){
    ...
    public function getAllBankTransactions(){
        ...
        $transactions = $this->db->query("
            select * from(
                ...
                select
                    'g' as sequence,
                    cp.SaleMaster_SlNo as id,
                    concat('Sale - ', c.Customer_Name, ' (', c.Customer_Code, ')') as description, 
                    cp.account_id,
                    cp.SaleMaster_SaleDate as transaction_date,
                    'deposit' as transaction_type,
                    cp.SaleMaster_PaidAmount as deposit,
                    0.00 as withdraw,
                    cp.SaleMaster_Description as note,
                    ac.account_name,
                    ac.account_number,
                    ac.bank_name,
                    ac.branch_name,
                    0.00 as balance
                from tbl_salesmaster cp
                join tbl_bank_accounts ac on ac.account_id = cp.account_id
                join tbl_customer c on c.Customer_SlNo = cp.SalseCustomer_IDNo
                where cp.account_id is not null
                ...
                select
                    'h' as sequence,
                    cp.PurchaseMaster_SlNo as id,
                    concat('Purchase - ', c.Supplier_Name, ' (', c.Supplier_Code, ')') as description, 
                    cp.account_id,
                    cp.PurchaseMaster_OrderDate as transaction_date,
                    'withdraw' as transaction_type,
                    0.00 as deposit,
                    cp.PurchaseMaster_PaidAmount as withdraw,
                    cp.PurchaseMaster_Description as note,
                    ac.account_name,
                    ac.account_number,
                    ac.bank_name,
                    ac.branch_name,
                    0.00 as balance
                from tbl_purchasemaster cp
                join tbl_bank_accounts ac on ac.account_id = cp.account_id
                join tbl_supplier c on c.Supplier_SlNo = cp.Supplier_SlNo
                where cp.account_id is not null
                and cp.payment_type = 'Bank'
                and cp.status = 'a'
