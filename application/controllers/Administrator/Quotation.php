...
class Quotation extends CI_Controller {
    ...
    public function getSalesQuotation(){
        ...
        try{
            ...
            $quotation_Id = $data->quotation->quotationId;
            ...
            $quotation = array(
                ...
                'SaleMaster_SaleDate' => $data->quotation->quotationDate,
                'SalseCustomer_IDNo' => $data->quotation->customerId,
                'SaleMaster_SaleType' => $data->quotation->salesType,
                'SaleMaster_TotalSaleAmount' => $data->quotation->total,
                'SaleMaster_TotalDiscountAmount' => $data->quotation->discount,
                'SaleMaster_TaxAmount' => $data->quotation->vat,
                'SaleMaster_Previous_Due' => $data->quotation->previousDue,
                'SaleMaster_SubTotalAmount' => $data->quotation->subTotal,
                'Status' => 'a',
                "AddBy" => $this->session->userdata("FullName"),
                'AddTime' => date("Y-m-d H:i:s"),
            ...
            $this->db->insert('tbl_salesmaster', $quotation);
            ...
            $this->db->query("
                ...
                ", [$quotation_Id, $this->session->userdata('BRANCHid')]);
            ...
            foreach($data->cart as $cartProduct){
                ...
                $this->db->insert('tbl_saledetails', $quotationDetails);
                $this->db->query("
            ...
            // $currentDue = $data->quotation->previousDue + ($data->sales->total - $data->sales->paid);
            $res = ['success'=>true, 'message'=>'Quotation Sales', 'quotationId'=>$quotationId];
        } catch (Exception $ex){
            $res = ['success'=>false, 'message'=>$ex->getMessage()];
        }
    ...
    public function getQuotations(){
        ...
        $res['quotations'] = $this->db->query("
            ...
            join tbl_customer c on c.Customer_SlNo = qm.SalseCustomer_IDNo
            where qm.Status = 'a'
    ...
    public function salesQuotation($quotationId){
        $invoice = $this->mt->generateSalesInvoice();
        $data['title'] = "Quotation Sales";
        $data['quotationId'] = $quotationId;
        $data['invoice'] = $invoice ;
        $data['content'] = $this->load->view('Administrator/quotation/quotation_sales', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
