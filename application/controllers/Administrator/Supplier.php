...
class Supplier extends CI_Controller {
    ...
    function supplierPaymentAndReport($id = Null)
    {
        $data['title'] = "Supllier Payment Reports";
        if ($id != 'pr') {
            $pid["PamentID"] = $id;
            $this->session->set_userdata($pid);
        }
        $data['content'] = $this->load->view('Administrator/due_report/supplierPaymentAndReport', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getSupplierLedger(){
        ...
        $payments = $this->db->query("
            select
                ...
                0.00 as damageAmount,
                0.00 as cash_discount,
                0.00 as balance
            ...
            select
                'e' as sequence,
                dr.Damage_SlNo as id,
                dr.Damage_Date as date,
                'Damage Product' as description,
                0.00 as bill,
                0.00 as paid,
                0.00 as due,
                0.00 as returned,
                0.00 as cash_received,
                dr.damage_amount as damageAmount,
                0.00 as cash_discount,
                0.00 as balance
            from  tbl_damage dr
            where dr.Supplier_Id = '$data->supplierId'
            and dr.status = 'a'
            ...
            select 
                ...
                0.00 as cash_received,
                0.00 as damageAmount,
                sp2.SPayment_discount as cash_discount,
                0.00 as balance
            from tbl_supplier_payment sp2
            left join tbl_bank_accounts ba on ba.account_id = sp2.account_id
            where sp2.SPayment_customerID = '$data->supplierId'
            and sp2.SPayment_discount != 0
            and sp2.SPayment_status = 'a'
        ...
        foreach($payments as $key=>$payment){
            ...
            $payment->balance = ($lastBalance + $payment->bill + $payment->cash_received) - ($payment->paid + $payment->returned + $payment->damageAmount+$payment->cash_discount);
        }
    ...
    public function getSupplierPayments(){
        ...
        $payments = $this->db->query("
            ...
            and sp.SPayment_brunchid = ? $paymentTypeClause $dateClause $clauses
    ...
    public function suppplierPaymentHistory(){
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
