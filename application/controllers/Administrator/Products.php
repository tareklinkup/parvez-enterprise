...
class Products extends CI_Controller {
    ...
    public function category_price_update()  {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Category Price Update";
        $data['content'] = $this->load->view('Administrator/products/category_price_update', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function add_category_price_update() {
        ...
        try{
            ...
            $product['status'] = 'a';
            $product['add_by'] = $this->session->userdata("FullName");
            $product['add_time'] = date('Y-m-d H:i:s');
            $product['branch_id'] = $this->brunch;

            $this->db->insert('tbl_category_product_price_update', $product);
            ...
            $products = $this->db->query(
                ...
                from tbl_product p
                where p.status = 'a'
                and p.ProductCategory_ID = '$productObj->category_id'
            ")->result();
            
            foreach($products as $p){
                $update_price = $p->Product_SellingPrice + (($p->Product_SellingPrice * $productObj->update_percent)/100);
                $update_wholesale_price = $p->Product_WholesaleRate + (($p->Product_WholesaleRate * $productObj->update_percent)/100);

                $this->db->where('Product_SlNo', $p->Product_SlNo)->update('tbl_product', [
                    'Product_SellingPrice' => $update_price,
                    'Product_WholesaleRate' => $update_wholesale_price,
                ]);
            }

            $res = ['success'=>true, 'message'=>'sale and wholesale price updated!'];
        } catch (Exception $ex){
    ...
    public function get_category_price_updates()
    {
        $products = $this->db->query(
            "SELECT
                p.*,
            ...
            from tbl_category_product_price_update p
            left join tbl_productcategory c on c.ProductCategory_SlNo = p.category_id
            left join tbl_brunch b on b.brunch_id = p.branch_id
            where p.status = 'a'
            order by p.id desc
        ")->result();
    ...
    public function getProducts(){
        ...
        foreach($products as $product){
            $product->stock = $this->mt->productStock($product->Product_SlNo);
        }
        echo json_encode($products);
    }
