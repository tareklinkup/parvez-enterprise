<?php
    class Graph extends CI_Controller{
        ...
        public function getGraphData(){
            ...
            $thisMonthPurchase = $this->db->query("
                select 
                    ifnull(sum(ifnull(sm.PurchaseMaster_TotalAmount, 0)), 0) as total_amount
                from tbl_purchasemaster sm
                where sm.status = 'a'
                and month(sm.PurchaseMaster_OrderDate) = ?
                and year(sm.PurchaseMaster_OrderDate) = ?
                and sm.PurchaseMaster_BranchID = ?
            ", [$month, $year, $this->branchId])->row()->total_amount;
            
            // This Year's Purchase
            $thisYearPurchase = $this->db->query("
                select 
                    ifnull(sum(ifnull(sm.PurchaseMaster_TotalAmount, 0)), 0) as total_amount
                from tbl_purchasemaster sm
                where sm.status = 'a'
                and year(sm.PurchaseMaster_OrderDate) = ?
                and sm.PurchaseMaster_BranchID = ?
            ", [$year, $this->branchId])->row()->total_amount;
            ...
            $monthly_collection = $this->db->query("
                select 
                ifnull((
                    select sum(ifnull(sm.SaleMaster_PaidAmount, 0)) 
                    from tbl_salesmaster sm
                ...
                ifnull((
                    ...
                    and cp.CPayment_TransactionType = 'CR'
                    and cp.CPayment_brunchid = " . $this->branchId . "
                    and month(cp.CPayment_date) = '" . $month . "'
                    and year(cp.CPayment_date) = '" . $year . "'
                ), 0) +
                ifnull((
                    select sum(ifnull(ct.In_Amount, 0)) 
                    from tbl_cashtransaction ct
            ...
            // yearly Cash Collection
            $yearly_collection = $this->db->query("
                select 
                ifnull((
                    select sum(ifnull(sm.SaleMaster_PaidAmount, 0)) 
                    from tbl_salesmaster sm
            ...
            $dueResult = $this->db->query(
                ...
                (
                    ...
                    and month(sr.SaleReturn_ReturnDate) = '" . $month . "'
                    and year(sr.SaleReturn_ReturnDate) = '" . $year . "'
                ) as returnedAmount,
                ...
                from tbl_customer c
                where c.Customer_brunchid = '$this->branchId'
            ")->result();

            $monthlyCustomerDue = array_sum(array_map(function($due) {
                return $due->dueAmount;
            }, $dueResult));
            ...
            $responseData = [
                ...
                'this_year_purchase'   => $thisYearPurchase,
                ...
                'monthly_customer_due' => $monthlyCustomerDue,
