...
class Sales extends CI_Controller {
    ...
    public function addSales(){
        ...
        try{
            ...
            $sales = array(
                ...
                'return_amount' => $data->sales->return_amount,
                'payment_type' => $data->sales->CPayment_Paymentby,
                'account_id' => $data->sales->account_id,
                'cash_amount' => $data->sales->cash_amount ?? 0,
                'entry_type' => $data->sales->entry_type,
                'Status' => 'a',
    ...
    public function pos_sales_edit($salesId){
        $data['title'] = "Pos Sales Update";
        $sales = $this->db->query("select * from tbl_salesmaster where SaleMaster_SlNo = ?", $salesId)->row();
        $data['salesId'] = $salesId;
        $data['invoice'] = $sales->SaleMaster_InvoiceNo;
        $data['content'] = $this->load->view('Administrator/sales/pos_sales', $data, TRUE);
    ...
    public function getSalesRecord(){
        ...
        if(isset($data->entry_type) && $data->entry_type != ''){
            $clauses .= " and sm.entry_type = '$data->entry_type'";
        }
    ...
    public function getSales(){
        ...
        if(isset($data->account_id) && $data->account_id != ''){
            $clauses .= " and sm.account_id = '$data->account_id'";
        }
...
//    }
    ...
    public function getProfitLoss(){
        ...
        }else if(($data->customer == null && $data->customer == '') && $data->searchTypes == 'listed'){
            $customerClause = " and c.Customer_Type != 'G'";
        }else if(($data->customer == null && $data->customer == '') && $data->searchTypes == 'general'){
            $customerClause = " and c.Customer_Type = 'G'";
        }
