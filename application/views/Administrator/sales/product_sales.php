						...
						<div class="col-12 col-sm-5 clearfix">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									...
									<div class="col-xs-9">
										<input type="number" id="salesRate" placeholder="Rate" step="0.0001" class="form-control" v-model="selectedProduct.Product_SellingPrice" v-on:input="productTotal"/>
									</div>
								</div>
								<div class="form-group">
									<label class="col-xs-3 control-label no-padding-right"> Quantity </label>
									<div class="col-xs-9">
										<input type="number" step="0.0001" id="quantity" placeholder="Qty" class="form-control" ref="quantity" v-model="selectedProduct.quantity" v-on:input="productTotal" autocomplete="off" required/>
									</div>
								</div>
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									...
									<tr>
										<td>
										<div class="form-group">
										<label class="col-xs-12 control-label no-padding-right">Receive Type</label>
													<div class="col-md-12">
														<select class="form-control" v-model="sales.CPayment_Paymentby" required>
															<option value="Cash">Cash</option>
															<option value="Bank">Bank</option>
														</select>
													</div>
												</div>
												<div class="form-group" style="display:none;" v-bind:style="{display: sales.CPayment_Paymentby == 'Bank' ? '' : 'none'}">
...
<script>
	...
	new Vue({
		...
		methods:{
			...
			async saveSales(){
				if(this.sales.CPayment_Paymentby == 'Bank'){
				...
				this.sales.cash_amount = this.sales.SaleMaster_PaidAmount;
			...
			async getSales(){
				await axios.post('/get_sales', {salesId: this.sales.salesId}).then(res=>{
					...
					this.sales.CPayment_Paymentby = sales.payment_type;
					...
					this.sales.entry_type = sales.entry_type;
					this.sales.return_amount = sales.return_amount;
					this.sales.cash_amount = sales.cash_amount;
