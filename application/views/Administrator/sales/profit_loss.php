...
<div id="profitLoss">
	<div class="row" style="border-bottom: 1px solid #ccc;">
		<div class="col-md-12">
			<form class="form-inline" id="searchForm" v-on:submit.prevent="getProfitLoss">
			<div class="form-group">
					<label>Search Type</label>
					<select class="form-control" style="padding: 0;" v-model="searchType" @change="onChangeSearchType">
						<option value="">All</option>
						<option value="general">General Customar</option>
						<option value="listed">Listed Customar</option>
					</select>
				</div>
				<div class="form-group" style="margin-right: 15px; display:none" v-bind:style="{display: searchType == 'listed' || searchType == '' && customers.length > 0 ? '' : 'none'}">
...
<script>
	...
	new Vue({
		...
		methods: {
			onChangeSearchType(){
				this.sales = [];
				if(this.searchType == 'listed' || this.searchType == '' ){
					this.getCustomers();
				}
			},
			getCustomers(){
			...
			async getProfitLoss(){
				if(this.selectedCustomer != null){
					...
					this.filter.searchTypes = this.searchType;
				} else{
					this.filter.customer = null;
					this.filter.searchTypes = null;
				}
