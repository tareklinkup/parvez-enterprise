			...
			<form @submit.prevent="saveSupplierPayment">
				<div class="row">
					...
					<div class="col-md-5">
						...
						<div class="form-group" v-if="payment.SPayment_TransactionType == 'CP'">
				...
				<datatable :columns="columns" :data="payments" :filter-by="filter" style="margin-bottom: 5px;">
					<template scope="{ row }">
						<tr>
							...
							<td>
							<button type="button" class="button edit" @click="window.location = `/supplierPaymentAndReport/${row.SPayment_id}`">
	...
	new Vue({
		...
		data(){
			return {
				...
				columns: [
                    ...
                    { label: 'Discount', field: 'SPayment_discount', align: 'center' },
		...
		methods:{
				...
				axios.post(url, this.payment).then(res => {
					...
					if(r.success){
						...
						if(invoiceConfirm == true){
							window.open('/supplierPaymentAndReport/'+r.paymentId, '_blank');
						}
					}
				})
			},
			...
			resetForm(){
				...
				this.payment.SPayment_discount = '';
