								...
								<table style="color:#000;margin-bottom: 0px;">
									...
									<tr>
										...
										<label class="col-xs-12 control-label no-padding-right">Receive Type</label>
													<div class="col-md-12">
														<select class="form-control" v-model="purchase.CPayment_Paymentby" required>
															<option value="Cash">Cash</option>
															<option value="Bank">Bank</option>
														</select>
													</div>
												</div>
												<div class="form-group" style="display:none;" v-bind:style="{display: purchase.CPayment_Paymentby == 'Bank' ? '' : 'none'}">
	...
	new Vue({
		...
		methods:{
			...
			savePurchase(){
				...
				if(this.purchase.CPayment_Paymentby == 'Bank'){
					if(this.selectedAccount == null){
						alert('Select an account');
						return;
					} else {
						this.purchase.account_id = this.selectedAccount.account_id;
					}
				} else {
			...
			async getPurchase(){
				await axios.post('/get_purchases', {purchaseId: this.purchase.purchaseId}).then(res => {
					...
					this.purchase.CPayment_Paymentby = purchase.payment_type;
