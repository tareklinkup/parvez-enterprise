					...
					<div class="row">
						...
						<div class="form-group">
							...
							<div class="form-group clearfix">
								...
								<div class="col-sm-8">
									<input type="text" class="form-control"  placeholder="Customer/Company Name" v-model="selectedCustomer.Customer_Name" v-bind:disabled="selectedCustomer.Customer_Type == 'G' ? false : true">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<textarea placeholder="Address" class="form-control"  v-model="selectedCustomer.Customer_Address" v-bind:disabled="selectedCustomer.Customer_Type == 'G' ? false : true"></textarea>
								</div>
							</div>
						</div>
...
<script>
	...
	new Vue({
		...
		methods:{
			...
			async getCustomers(){
				await axios.post('/get_customers', {customerType: this.quotation.salesType}).then(res=>{
			...
			getQuotations(){
				axios.post('/get_quotations', {quotationId: this.quotation.quotationId}).then(res=>{
					...
					this.selectedCustomer = {
						...
						display_name: quotation.Customer_Type == 'G' ? 'General Customer' : `${quotation.Customer_Code} - ${quotation.Customer_Name}`,
