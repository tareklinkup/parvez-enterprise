...
class Model_Table extends CI_Model{
    ...
    public function getBankTransactionSummary($accountId = null, $date = null){
        $bankTransactionSummary = $this->db->query("
            select 
                ...
                (
                    select ifnull(sum(cp.SaleMaster_PaidAmount), 0) from tbl_salesmaster cp
                    where cp.account_id = ba.account_id
                    and cp.Status = 'a'
                    and cp.payment_type = 'Bank'
                    and cp.SaleMaster_branchid = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and cp.SaleMaster_SaleDate < '$date'") . "
                ) as sale_amount,
                (
                    select ifnull(sum(cp.PurchaseMaster_PaidAmount), 0) from tbl_purchasemaster cp
                    where cp.account_id = ba.account_id
                    and cp.status = 'a'
                    and cp.payment_type = 'Bank'
                    and cp.PurchaseMaster_BranchID = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and cp.PurchaseMaster_OrderDate < '$date'") . "
                ) as purchase_amount,
                (
                    select ifnull(sum(cp.CPayment_amount), 0) from tbl_customer_payment cp
                    where cp.account_id = ba.account_id
                ...
                (
                    select (ba.initial_balance + sale_amount + total_deposit + total_received_from_customer + total_received_from_supplier) - (total_withdraw + purchase_amount + total_paid_to_customer + total_paid_to_supplier)
                ) as balance
    ...
    public function supplierDue($clauses = "" , $date = null) {
        ...
        $supplierDues = $this->db->query("
            ...
            (select ifnull(sum(sp2.SPayment_discount), 0.00) from tbl_supplier_payment sp2 
                where sp2.SPayment_customerID = s.Supplier_SlNo
                " . ($date == null ? "" : " and sp2.SPayment_date < '$date'") . "
                and sp2.SPayment_status = 'a'
            ) as cashDiscount,
            ...
            (select ifnull(sum(dr.damage_amount), 0.00) from tbl_damage dr
                where dr.Supplier_Id = s.Supplier_SlNo
                " . ($date == null ? "" : " and dr.Damage_Date < '$date'") . "
                and dr.status = 'a'
            ) as damageAmount,
            
            (select invoicePaid + cashPaid + damageAmount + cashDiscount) as paid,
